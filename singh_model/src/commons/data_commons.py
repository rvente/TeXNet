...
def initialize(training_data_dir, params):
    ...
    if i2w_ufunc is None:
        ...
        K = len(list(dict_id2word.keys()))
        CTCBlankTokenID = params.CTCBlankTokenID
        if (CTCBlankTokenID is not None) and (CTCBlankTokenID >= K):
            dict_id2word[CTCBlankTokenID] = '<>' ## CTC Blank Token
        dict_id2word[-1] = '<-1>' ## Catch -1s that beamsearch emits after EOS.
        def i2w(id):
            try:
...
def seq2str(arr, label, separator=None):
    ...
    if separator is None:
        func1d = lambda vec: label + " " + "".join(vec)
    else:
        func1d = lambda vec: label + " " + str(separator).join(vec)
