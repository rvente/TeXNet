...
class VisualizeDir(object):
    ...
    def strs(self, graph, step, key, key2=None, mingle=False, trim=True, wrap_strs=False, sortkey='ed', keys=[],
        ...
        data = {'%s_len' % key: ar1_len, '_iloc': list(range(df1.shape[0])), key: ar1}
    ...
    def prune_logs(self, save_epochs=1, dry_run=True):
        ...
        if len(epoch_steps) <= save_epochs:
            ...
            steps_to_remove = set([s for s in training_steps if (s<max_step) and (s not in epoch_steps)])
    ...
    def prune_snapshots(self, keep_first=None, keep_last=None, dry_run=True):
        ...
        if keep_last is not None:
            steps_to_keep = set([step for step in steps if (keep_first <= step <= keep_last)])
        else:
            steps_to_keep = set([step for step in steps if (keep_first <= step)])
...
class EvalRuns(dlc.Properties):
    ...
    def _make_df(self, hyper_names, arg_names, metric_names):
        ...
        self.hypers = {}
        self.hypers.update(list(zip(self._logdirs, hypers)))

        args = [self.load_args(logdir) for logdir in self._logdirs]
        self.args = {}
        self.args.update(list(zip(self._logdirs, args)))

        metrics_n_cols = [self.get_metrics(logdir, hypers[i], metric_names) for (i, logdir) in enumerate(self._logdirs)]
        metric_cols = list(zip(*metrics_n_cols))[1]
        metrics = list(zip(*metrics_n_cols))[0]
        self.metrics = {}
        self.metrics.update(list(zip(self._logdirs, metrics)))
        ...
        for i, logdir in enumerate(self._logdirs):
            ...
            for tag in list(metrics[i].keys()):
    ...
    def get_metrics(self, logdir, hyper, metric_names):
        ...
        def insert_vals(row, df, tag_exp, op, selected_id=None):
            ...
            colsort = []
            tags = [tag for tag in all_tags if bool(re.search(tag_exp, tag))]
            split_op = op.split('_')
