...
%\begin{figure}[!h]
\begin{figure}[tbp]
		\includegraphics[scale=1.7]{harvest.pdf}
    \centering
...
\begin{figure}[h]
	...
	\begin{subfigure}{1.0\textwidth}
		...
		\caption{}
	\end{subfigure}
  \caption[Model]{This assessment is on the validation set (a) The mean length
  is with a mean of  32.56 and standard deviation of 23.82. (b) BLEU Scores are
  strongly concentrated on 1.0, with $\mu = 92.5$ and $\sigma =  23.81$.
  (c) As expected, performance is inversely proportional to length, however on the longest
  tokens, the model still perfomed at a respectable .82 BLEU score.}
\end{figure}
...
\begin{figure}[h]
	...
	\begin{subfigure}{0.5\textwidth}
		...
		\caption{}
	\end{subfigure}
  \caption[Model]{(a) We generalize cases, arrays, matrices, column vectors here
  and evaluate performance on that set finding $\mu = 0.85, \sigma=0.14$. (b) We match
  \texttt{frac} and \texttt{over} for fractions finding $\mu = 0.91, \sigma =.11$.}
\end{figure}
...
and \parencite{deng2016you}, the bigram BLEU (BLEU2) score computed with $w_1=w_2=.5$
and $w_i = 0$ for $i>2$. The scores we report actually compare non-normalized
target sequences to the normalized predicted sequences, resulting in a
pessimistic predictive score because the network was not trained on this
sequence. Despite this, the BLEU2 scores are satisfactory.
...
match. Math code encased in dollar-signs has been deprecated in \LaTeX{}, but
remains in wide use as an artifact from \TeX{}. It was deprecated to facilitate
parsing: unlike \verb|\(| which indicates that the expression opens to the
right, the dollar-sign gives no such indication. The reason the previous scripts
were matching these plain-text strings was because matching all strings that
start and end in dollar-signs doesn't preclude text from matching. In this case,
the \verb|$| of the first line matched with the first \verb|$| of the 4th line. 
